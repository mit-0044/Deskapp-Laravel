                ...
                <div class="pd-10 card-box mb-20 mt-20">
                    <div class="col-md-12 pl-0 my-2">
                        <div class="d-flex justify-content-between mb-20 col-md-12">
                            <h3 class="text-blue">Update Trasaction</h3>
                        </div>
                    </div>
                                                    ...
                                                    @if ($projects)
                                                        @foreach ($projects as $project)
                                                            <option value="{{ $project->id }}"
                                                                @if ($project->id == $transaction->project_id) selected @endif>
                                                                {{ $project->name }}
                                                            </option>
                                                        @endforeach
                                                    @endif
                                                    ...
                                                    @if ($transaction_types)
                                                        @foreach ($transaction_types as $transaction_type)
                                                            <option value="{{ $transaction_type->id }}"
                                                                @if ($transaction_type->id == $transaction->transaction_type_id) selected @endif>
                                                ...
                                                <select class="custom-select" name="income_source_id">
                                                    ...
                                                    @if ($income_sources)
                                                        @foreach ($income_sources as $income_source)
                                                            <option value="{{ $income_source->id }}"
                                                                @if ($income_source->id == $transaction->income_source_id) selected @endif>
                                                                {{ $income_source->name }}
                                                            </option>
                                                        @endforeach
                                                    @endif
                                                    ...
                                                    @if ($currencies)
                                                        @foreach ($currencies as $currency)
                                                            <option value="{{ $currency->id }}"
                                                                @if ($currency->id == $transaction->currency_id) selected @endif>
                                                                {{ $currency->name }}
                                                            </option>
                                                        @endforeach
                                                    @endif
                                        ...
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                ...
                                                <textarea class="form-control" type="text" placeholder="Description" name="description"
                                                    value="{{ $transaction->description }}" /></textarea>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="row">
