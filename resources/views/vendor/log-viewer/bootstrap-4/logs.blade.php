    ...
    <div class="pd-ltr-20 xs-pd-20-10">
        <div class="min-height-200px">
            <div class="page-header mb-4 d-flex justify-content-between">
                ...
                <h3 class="text-primary">@lang('Log Details')</h3>
                            ...
                            <tr>
                                @foreach ($headers as $key => $header)
                                    <th scope="col" class="{{ $key == 'date' ? 'text-left' : 'text-center' }}">
                                        @if ($key == 'date')
                                            <span
                                                class="badge badge-info my-auto btn-lg px-4 ml-2">{{ $header }}</span>
                                        @else
                                ...
                                <th scope="col" class="text-right">
                                    <h3 class="text-dark">@lang('Actions')</h3>
                                </th>
                            </tr>
                        </thead>
                        <tbody>
                                    ...
                                    @foreach ($row as $key => $value)
                                        <td class="{{ $key == 'date' ? 'text-left' : 'text-center' }}">
                                            @if ($key == 'date')
                                                <div class="">
                                                    <span
                                                        class="badge badge-primary">{{ date('d-m-Y', strtotime($value)) }}</span>
                                                </div>
                                            @elseif ($value == 0)
    ...
    <script>
        ...
        $('.check_all').click(function() {
            if ($(this).prop('checked')) {
                $('.log_date').attr('checked', true);
            } else {
                $('.log_date').attr('checked', false);
            }
        });
    </script>
@endsection
